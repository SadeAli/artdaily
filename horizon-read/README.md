...
Run it: any static server rooted at the repo one level up, e.g.
`python3 -m http.server 8080` there, then `http://localhost:8080/horizon-read/`
— rooted at this folder instead, the page's SDK, chrome and font all sit
above the server's root and none of them load. No build, no deps.
